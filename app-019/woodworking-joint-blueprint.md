...
7. **打印 1:1**：A4 按块分页输出（三视图各起一页、切割步骤独立起页），每页页眉带方案名、榫卯类型、页别与「第 N 页/共 M 页」；内置 100mm 校验尺可用直尺实测验证缩放；齿形 1:1 模板页超 A4 宽时自动按 188mm 切片、多页拼贴（不缩放）；燕尾（含半隐）额外输出齿号索引页，逐行列出齿顶宽/齿根宽/中心位置，编号与图中圆标一一对应。
...
#/plan/:id/print    打印视图（三视图分页 + 1:1 模板页 + 校验尺 + 燕尾齿号索引 + 切割步骤）
#/library           榫卯知识卡
```
...
- 打印：`@media print` 隐藏 `.no-print`（顶栏与打印工具栏）；`@page` 固定 A4 纵向、10mm 留白；打印内容按块分页——三个视图各自独立起页（放不下一页时自动缩到页内，1:1 模板页不缩，超宽时提示需拼贴/A3），1:1 模板页、校验尺、燕尾齿号索引、切割步骤各起一页；每页页眉为「方案名 · 榫卯类型 · 页别 · 第 N 页/共 M 页」，装订前可知总页数。燕尾（含半隐）额外输出齿号索引页，逐行列出齿顶宽/齿根宽/中心位置（齿左缘+齿顶宽÷2），编号与视图圆标一一对应；步骤与表格行 `break-inside: avoid`。页面上写清「关闭适应页面/缩放，100% 打印」。
...
- **测试总量**：vitest 5 个文件 60 例 = 单元 43（dovetail 8 / tenon 13 / views 14 / store 8）+ 组件 17；Playwright E2E 7 例。
- **打印**：页面含 100mm 校验尺，实测 0→100 段误差 ≤ 1mm；打印块按页分割（三视图各一页、模板/校验尺/齿号索引/切割步骤独立起页），页眉含方案名、榫卯类型、页别与「第 N 页/共 M 页」；燕尾齿号索引逐行列齿顶宽、齿根宽、中心位置且编号与圆标一致；打印调用与模板页可被 E2E 断言。
...
5. **`Scale` 与多榫卯只有模型没有界面**：`Drawing.scale` 恒为 `'1:1'`（`store/plans.ts:95`），`Part.jointIds` 恒为空数组（`plans.ts:91-92`）；编辑器只渲染 `joints[0]`（`EditorPage.tsx:40`），方案筛选同样只看 `joints[0]`（`plans.ts:42`），打印视图按实际尺寸出（`EditorPage.tsx:274`），因此 1:2 / 1:5 出图与单方案多榫卯均未实现。
